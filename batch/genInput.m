...
x_values = [0.4];
angles = [30, 45, 60];
disp_values = [-1.2e-3, -1e-3, -8e-4, -6e-4];

n_cases = 3;
count = 0;

%ANISOTROPIC CASES
for da = 1:length(angles)
    params.ang = angles(da);
    for i = 1:length(x_values)
        ...
        for case_num = 1:n_cases
            ...
            for d = 1:length(disp_values)
                ...
                params.saveName = sprintf("Anisotrop_x=%02d_case%d_ang=%2d_disp=%.0fe-4", ...
                    round(x*10), case_num, params.ang, -disp_val*1e4);
